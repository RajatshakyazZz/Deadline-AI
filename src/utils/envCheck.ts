...
export function validateEnvironment() {
  ...
  const geminiKey = getEnvValue('VITE_GEMINI_API_KEY') || safeStorage.getItem('deadlineai_user_gemini_key');
  if (!geminiKey) {
    missing.push('VITE_GEMINI_API_KEY');
  }

  // Check essential Firebase credentials from environment variables or the config file
  const apiKey = getEnvValue('VITE_FIREBASE_API_KEY') || firebaseAppletConfig.apiKey;
  const projectId = getEnvValue('VITE_FIREBASE_PROJECT_ID') || firebaseAppletConfig.projectId;
  const appId = getEnvValue('VITE_FIREBASE_APP_ID') || firebaseAppletConfig.appId;
...
export function getGeminiKey(): string {
  const key = getEnvValue('VITE_GEMINI_API_KEY') || safeStorage.getItem('deadlineai_user_gemini_key');
